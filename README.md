...
Gemini responses that stop for `MAX_TOKENS` or another non-`STOP` reason are rejected, not saved as successful transcripts. Stage 1 also rejects an exact 8-word sequence repeated 50+ times, catching model loops that incorrectly finish with `STOP`. For an unusually dense lesson, retry that file with `--retranscribe` and shorter `COURSE_TO_MD_CHUNK_THRESHOLD_SEC` / `COURSE_TO_MD_CHUNK_LENGTH_SEC` values.
